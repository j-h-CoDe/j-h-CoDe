...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, delta=0, path='checkpoint_3.pt'):
        """
        Args:
...
def experiment(model, train_loader, test_loader, device, args):
    ...
    for epoch in range(args.epoch):
        ...
        if epoch > 20:
            ...
            model.load_state_dict(torch.load('checkpoint_3.pt'))
    ...
    save_checkpoint(args.epoch, model, optimizer, './mymodel_3.pt')  # Pass epoch instead of epoch variable
...
pred_data_total = pd.DataFrame(pred_data_total, columns=['3']) 
pred_data_total = pd.concat([test_df[test_df.columns[0:2]], pred_data_total],axis=1)

iter_num = "Iter_3"
res.to_csv(f'./Result/{iter_num}/result_3.csv',index=False)
pred_data_total.to_csv(f"./Result/{iter_num}/alphafold_prediction_3.csv",index=False)
