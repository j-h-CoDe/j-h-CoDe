...
# 파이썬 스크립트 실행
subprocess.run(["python", "master_random_run.py"])

# stdout과 stderr를 파일로 리다이렉션하려면:
with open("master_random_run", "w") as out, open("master_random_run.err", "w") as err:
    subprocess.run(["python", "master_random_run.py"], stdout=out, stderr=err)
