...
# 파이썬 스크립트 실행
subprocess.run(["python", "2023_GCN_Regression_Model_0.py"])

# stdout과 stderr를 파일로 리다이렉션하려면:
with open("AINB_0", "w") as out, open("0.err", "w") as err:
    subprocess.run(["python", "2023_GCN_Regression_Model_0.py"], stdout=out, stderr=err)
